...
jouer_pc(Grille, L, Coup, Max_val) ->
	...
	Prof = 9,
	Val = algo_min(NouvelleGrille, L2, Prof, 1000, -1000),
	io:format("~p:R#~p.~n",[Prof+1, Val]),
...
algo_min(Grille, [], N, Min_val, _) ->
	case verifier(Grille) of
		{victoire, _} ->
			io:format("~p:F#~p.~n",[N, eval(Grille)]),
			eval(Grille);
		manque ->
			%io:format("F:~p~n",[Min_val]),
			Min_val
	end;

algo_min(Grille, _, 0, _, _) ->
	io:format("0:F#~p.~n",[eval(Grille)]),
	eval(Grille);
...
algo_max(Grille, [], N, _, Max_val) ->
	case verifier(Grille) of
		{victoire, _} ->
			io:format("~p:F#~p.~n",[N, eval(Grille)]),
			eval(Grille);
		manque ->
			%io:format("F:~p~n",[Max_val]),
			Max_val
	end;
